				...
				qb.where("note.replyId IS NULL") // 返信ではない
					.orWhere(
						new Brackets((qb) => {
							...
							).andWhere("((reply.replyUserId IS NULL) OR (reply.replyUserId = note.userId))");
						}),
					);
			}),
		);
					...
					qb.where("note.replyId IS NULL") // 返信ではない
						...
						.orWhere(
							new Brackets((qb) => {
								...
								).andWhere(`((reply.replyUserId IS NULL) OR (reply.replyUserId = note.userId) OR (reply.replyUserId IN (${following}) AND note.userId IN (${following})))`);
							}),
						)
						.orWhere(
							new Brackets((qb) => {
