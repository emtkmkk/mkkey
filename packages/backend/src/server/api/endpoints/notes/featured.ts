...
export default define(meta, paramDef, async (ps, user) => {
	...
	const query = Notes.createQueryBuilder("note")
		...
		.andWhere("note.id = ANY (SELECT note2.id FROM note note2 WHERE note.score > 0 AND note2.\"userId\" = note.\"userId\" AND date_trunc('day',note2.\"createdAt\") = date_trunc('day',note.\"createdAt\") ORDER BY note2.score DESC LIMIT 2)")
