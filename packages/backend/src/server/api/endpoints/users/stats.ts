...
export default define(meta, paramDef, async (ps, me) => {
	...
	const result = await awaitAll({
		...
		totalWordCount: (await Notes.createQueryBuilder("note")
			.select("coalesce(sum(length(regexp_replace(regexp_replace(note.text,':[\w_]+?:','☆', 'g'),'<\/?[\w_]+>|\$\[\S+\s|https?:\/\/[\w/:%#\$&@\?\(\)~\.=\+\-]+|@[\w_]+|#[\w_]+','', 'ig'))),0) + coalesce(sum(length(regexp_replace(regexp_replace(note.cw,':[\w_]+?:','☆', 'g'),'<\/?[\w_]+>|\$\[\S+\s|https?:\/\/[\w/:%#\$&@\?\(\)~\.=\+\-]+|@[\w_]+|#[\w_]+','', 'ig'))),0) count")
			.where("note.userId = :userId", { userId: user.id })
			.cache(CACHE_TIME)
			.getRawOne()).count,
	});
