...
export default define(meta, paramDef, async (ps, me) => {
	...
	let now = new Date();
	let mkk = new Date(2023,3,5);
	
	const elapsedDays = Math.max(Math.min(Math.ceil((now.getTime() - Date.parse(user.createdAt)) / (1000 * 60 * 60 * 2.4)) / 10,Math.ceil((now.getTime() - mkk.getTime()) / (1000 * 60 * 60 * 2.4)) / 10),1);
	...
	if (powerMkk >= rankBorder.slice(-1)[0] + suffixIncBorder) {
	    ...
	    result.nextRank = Math.floor((powerMkk - clearBorderMax) / ((rankBorder[clearBorder.length] ?? (clearBorder.slice(-1)[0] + suffixIncBorder)) - clearBorderMax) * 100) + "%"
	}
