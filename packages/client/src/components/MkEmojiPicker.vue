			...
			<div ref="emojis" class="emojis">
				...
				<div v-if="!$store.state.hiddenReactionDeckAndRecent && tab === 'index' && searchResultCustom.length <= 0 && (q == null || q === '')" class="group index">
					<section v-if="showPinned">
						<div class="body">
							<button
								v-for="emoji in pinned.filter((x) => (remoteEmojiMode === 'all' && props.asReactionPicker) || (remoteEmojiMode === 'plus' && props.asReactionPicker && plusEmojiStr.includes(x)) || !x.includes('@'))"
								:key="emoji"
								class="_button item"
								tabindex="0"
								@click="chosen(emoji, $event)"
					...
					<section>
						...
						<div class="body">
							<button
								v-for="emoji in recentlyUsedEmojis.filter((x) => (remoteEmojiMode === 'all' && props.asReactionPicker) || (remoteEmojiMode === 'plus' && props.asReactionPicker && plusEmojiStr.includes(x)) || !x.includes('@'))"
								:key="emoji"
								class="_button item"
								@click="chosen(emoji, $event)"
							>
...
const plusEmojiStr = props.asReactionPicker && remoteEmojiMode === "plus" ? allCustomEmojis.map((x) => ":" + x.name + "@" + x.host + ":") : undefined;
