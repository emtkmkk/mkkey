...
const handleImgClick = (event) => {
	if (props.note && defaultStore.state.noteReactionMenu && urlRaw.value.length <= errorCnt) {
			// TODO: 押せるか押せないかの判定を行えるように
		openReactionMenu_(props.emoji, props.note, true, true, event);
	} else if (props.note && defaultStore.state.noteQuickReaction && urlRaw.value.length <= errorCnt){
		const el =
			ev &&
