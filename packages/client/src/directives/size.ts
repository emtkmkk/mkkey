...
function calc(el: Element) {
	...
	if (
		currentTime - info.lastClassChangeTime < 1000 &&
		info.classChangeCount >= 5
	) {
		return; // Stop further changes
	}

	if (currentTime - info.lastClassChangeTime >= 1000) {
		info.classChangeCount = 0; // Reset the count every second
		info.lastClassChangeTime = currentTime;
	}

	info.classChangeCount += 1;
...
export default {
	...
	updated(src, binding, vn) {
		...
		if (info) {
			mountings.set(src, Object.assign({}, info, { value: binding.value }));
			calc(src);
		}
	},
