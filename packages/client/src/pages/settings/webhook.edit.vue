		...
		<FormSection>
			...
			<template
				v-if="
					event_antenna &&
					antennas.length > 0 &&
					event_excludeAntennas.length > 0
				"
			>
				<FormSection>
					<template #label
					...
					>
						<FormSwitch
							v-if="event_excludeAntennas.length > index"
...
let secret = $ref(
	webhook.secret?.startsWith("Discord") || webhook.secret?.startsWith("Slack")
		? ""
		: webhook.secret
);
...
let text_length = $ref(
	webhook.secret?.startsWith("Discord") || webhook.secret?.startsWith("Slack")
		? webhook.secret.replaceAll("Discord", "").replaceAll("Slack", "")
		: ""
);
